<template>
<div class="login row justify-content-center align-items-center p-0 m-0" :style="{
    ...
    <div class="login-body d-flex px-1 pt-2 pb-3 rounded position-relative">
        ...
        <div class="login-right d-flex flex-column align-items-center font-weight-bold px-2 px-md-4">
            <h2 class="mb-2 mt-2">Welcome</h2>
            <div class="d-block d-md-none">
                <span @click="linkTo('/')">xiaohuohu</span>
            </div>
            <div class="text-center mb-4 small">
                <span class="mx-2 px-3 py-1 btn btn-sm" :class="{ 'login-cho': isLoginOrLogon }" @click="isLoginOrLogon = true">登录</span>
                <span class="mx-2 px-3 py-1 btn btn-sm" :class="{ 'login-cho': !isLoginOrLogon }" @click="isLoginOrLogon = false">注册</span>
            </div>
            <!-- 登录 -->
            <div class="w-100">
                <Form ref="formlogin" label-position="top" key='formlogin' class="w-100" v-if="isLoginOrLogon" :model="loginValDate" :rules="loginRuleValidate">
                    ...
                    <FormItem class="text-center">
                        <Button type="success" long class="mb-3" @click="handleSubmit('formlogin')">登录</Button>
                        <Button type="error" long ghost @click="handleReset('formlogin')">清空</Button>
                    </FormItem>
                </Form>
                <Form ref="formlogon" label-position="top" key="formlogon" class="w-100" v-if="!isLoginOrLogon" :model="logonValDate" :rules="logonRuleValidate">
                    ...
                    <FormItem label="重复密码" prop="passagain">
                        <Input placeholder="password again" v-model="logonValDate.passagain"></Input>
                    </FormItem>
                    <FormItem class="text-center">
                        <Button type="success" long class="mb-3" @click="handleSubmit('formlogon')">登录</Button>
