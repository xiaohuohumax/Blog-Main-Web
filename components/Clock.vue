...
<div class="clock d-flex align-items-center justify-content-center mx-auto">
    <div class="appAll position-relative" :style="`--alldeg:${deg}deg`" ref="appAll">
        ...
        <div class="appHour appTimeBlock" :style="`--deg:-${hour*15}deg`">
            <div class="appTimeSon w-50 text-right position-absolute" v-for="(item, index) in 24" :key="index" :style="`--i:${index*15}deg`" :class="{'text-danger':index == hour}">{{index}}</div>
        </div>
        <!-- 分钟 -->
        <div class="appMinute appTimeBlock" :style="`--deg:-${minute*6}deg`">
            <div class="appTimeSon w-50 text-right position-absolute" v-for="(item, index) in 60" :key="index" :style="`--i:${index*6}deg`" :class="{'text-danger':index == minute}">{{index}}</div>
        </div>
        <!-- 秒 -->
        <div class="appAsecond appTimeBlock" :style="`--deg:-${second*6}deg`">
            <div class="appTimeSon w-50 text-right position-absolute" v-for="(item, index) in 60" :key="index" :style="`--i:${index*6}deg`" :class="{'text-danger':index == second}">{{index}}</div>
        </div>
        <!-- 吃豆人脸 -->
